...
// ** Actions Imports
import { fetchData } from 'src/store/apps/deduction'
import { DeductionsType } from 'src/local-db'
...
const DeductionsList = () => {
  ...
  const store = useSelector(state => state.deduction)
  ...
  const addDeduction = () => {
    router.push('/company-dashboard/payroll/deduction/add-deduction')
  }
  ...
  const RowOptions = ({ row }) => {
    ...
    const handleEditRowOptions = () => {
      router.push('/company-dashboard/payroll/deduction/' + row._id)
    ...
    return (
      <>
        ...
        >
          ...
          {session && session.user && session.user.permissions.includes('EditPayrollDeduction') && (
            <MenuItem onClick={handleEditRowOptions} sx={{ '& svg': { mr: 2 } }}>
              <Icon icon='mdi:pencil-outline' fontSize={20} />
              Edit
            </MenuItem>
          )}
          {session && session.user && session.user.permissions.includes('DeletePayrollDeduction') && (
        ...
        <Card>
          ...
          <Grid container spacing={6} sx={{ px: 5, pt: 3 }}>
            ...
            <Grid item sm={5} xs={12} textAlign={right}>
              ...
              {session && session.user && session.user.permissions.includes('AddPayrollDeduction') && (
