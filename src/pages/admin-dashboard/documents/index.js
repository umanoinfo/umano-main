    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          {session && session.user && session.user.permissions.includes('AdminEditDocumentType') && (
            <IconButton onClick={() => handleAdminEditDocumentType(row)}>
              <Icon icon='mdi:pencil-outline' />
            </IconButton>
          )}
          {row.deleted_at && session && session.user && session.user.permissions.includes('AdminDeleteDocumentType') && (
        ...
        <Grid item xs={12}>
          <Card>
            ...
            <Grid item xs={12}>
              ...
              >
                <Box />
                {session && session.user && session.user.permissions.includes('AdminAddDocumentType') && (
