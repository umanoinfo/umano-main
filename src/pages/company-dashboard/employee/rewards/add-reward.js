...
import { EmployeeRewardsType } from 'src/local-db'
...
const AddDepartment = ({ popperPlacement, id }) => {
  ...
  const handleSubmit = () => {
    formRef.current.checkAsync().then(result => {
      if (!result.hasError) {
        ...
        axios
          .post('/api/employee-reward/add-reward', {
            data
          })
          .then(function (response) {
            router.push('/company-dashboard/employee/rewards')
            toast.success('Reward (' + data.title + ') Inserted Successfully.', {
              delay: 3000,
  ...
  if (session && session.user && !session.user.permissions.includes('AddEmployeeReward'))
    return <NoPermission header='No Permission' description='No permission to add  employee reward'></NoPermission>
