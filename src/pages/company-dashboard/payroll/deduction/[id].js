...
const AddDepartment = ({ popperPlacement, id }) => {
  ...
  const handleSubmit = () => {
    formRef.current.checkAsync().then(result => {
      if (!result.hasError) {
        ...
        setLoading(true)
        setLoadingDescription('Deduction is updating')
        axios
          .post('/api/deduction/edit-deduction', {
            data
          })
          .then(function (response) {
            router.push('/company-dashboard/payroll/deduction')
            toast.success('Deduction (' + data.title + ') updated Successfully.', {
              delay: 3000,
  ...
  if (session && session.user && !session.user.permissions.includes('EditPayrollDeduction'))
    return <NoPermission header='No Permission' description='No permission to edit payroll deduction'></NoPermission>
