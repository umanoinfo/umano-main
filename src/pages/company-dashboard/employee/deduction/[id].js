...
const AddDepartment = ({ popperPlacement, id }) => {
  ...
  const handleSubmit = () => {
    formRef.current.checkAsync().then(result => {
      if (!result.hasError) {
        ...
        axios
          .post('/api/employee-deduction/edit-deduction', {
            data
          })
          .then(function (response) {
            router.push('/company-dashboard/employee/deduction')
            toast.success('Deduction (' + data.title + ') Edited Successfully.', {
              delay: 3000,
  ...
  if (session && session.user && !session.user.permissions.includes('EditEmployeeDeduction'))
    return <NoPermission header='No Permission' description='No permission to edit employee deduction'></NoPermission>
