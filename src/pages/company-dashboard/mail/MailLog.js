...
const MailLog = props => {
  ...
  const renderMailLabels = type => {
    return (
      <Box component='span' sx={{ mr: 3.5, color: `${labelColors[type]}.main` }}>
  ...
  const selectMail = id => {
    if (store && store.data.length > 0) {
      const mail = store.data.find(mail => mail.id == id)
      setMail(mail)
    }
  }
                    ...
                    >
                      <Box sx={{ mr: 4, display: 'flex', overflow: 'hidden', alignItems: 'center' }}>
                        <IconButton
                          ...
                          sx={{
                            mr: { xs: 0, sm: 3 },
                            color: mail.status == 'failed' ? 'error.main' : 'success.main',
                            '& svg': {
                        ...
                        >
                          ...
                          >
                            {mail.employee_info[0].firstName} {mail.employee_info[0].lastName}
                          </Typography>
                      ...
                      >
                        {mail.status == 'failed' ? (
                          <span>
                            ...
                            <Tooltip placement='top' title='Resend Mail'>
                              <IconButton
                                onClick={e => {
                                  // resend
                                }}
                      ...
                      >
                        <Box sx={{ display: { xs: 'none', sm: 'flex' } }}>{renderMailLabels(mail.type)}</Box>
                        <Typography
                          variant='caption'
